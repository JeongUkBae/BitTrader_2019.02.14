...
<div class="grid-item" id="content">
	...
	<div class="center">
		  ...
		  <c:if test="${pagination.existPrev}">
		    <a href='${ctx}/customer.do?cmd=cust_list&page=list&page_size=5&page_num=${pagenation.prevBlock}'>&laquo;</a>
		  </c:if>
		
		  <c:forEach begin="${pagination.startPage}" end="${pagination.endPage}" varStatus="status">
		  <a class="count" href="#"  >${status.index}</a>
		  </c:forEach>
		  
		  <c:if test="${pagination.existNext}">
		  <a href='${ctx}/customer.do?cmd=cust_list&page=list&page_size=5&page_num=${pagenation.nextBlock}'>&raquo;</a>
		  </c:if>
	</div>
